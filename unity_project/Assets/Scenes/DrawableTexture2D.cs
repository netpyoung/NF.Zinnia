...
public class DrawableTexture2D
{
    public Texture2D _texture;

    public DrawableTexture2D(Texture2D texture)
    {
        _texture = texture;
    }
    ...
    public void DrawLine(Vector2 from, Vector2 to, float w, Color col)
    {
        DrawLine(from, to, w, col, false, Color.black, 0);
    }

    public void DrawLine(Vector2 from, Vector2 to, float w, Color col, bool stroke, Color strokeCol, float strokeWidth)
    {
        ...
        var stY = Mathf.Clamp(Mathf.Min(from.y, to.y) - extent, 0, _texture.height);//This is the topmost Y value
        var stX = Mathf.Clamp(Mathf.Min(from.x, to.x) - extent, 0, _texture.width);
        var endY = Mathf.Clamp(Mathf.Max(from.y, to.y) + extent, 0, _texture.height);
        var endX = Mathf.Clamp(Mathf.Max(from.x, to.x) + extent, 0, _texture.width);//This is the rightmost Y value
        ...
        Color[] pixels = _texture.GetPixels((int)stX, (int)stY, (int)lengthX, (int)lengthY, 0);//Get all pixels
        ...
        _texture.SetPixels((int)stX, (int)stY, (int)lengthX, (int)lengthY, pixels, 0);
        _texture.Apply();
    }
    ...
    public void Paint(Vector2 pos, float rad, Color col, float hardness)
    {
        var start = new Vector2(Mathf.Clamp(pos.x - rad, 0, _texture.width), Mathf.Clamp(pos.y - rad, 0, _texture.height));
        var width = rad * 2;
        var end = new Vector2(Mathf.Clamp(pos.x + rad, 0, _texture.width), Mathf.Clamp(pos.y + rad, 0, _texture.height));
        var widthX = Mathf.Round(end.x - start.x);
        var widthY = Mathf.Round(end.y - start.y);
        var sqrRad = rad * rad;
        var sqrRad2 = (rad + 1) * (rad + 1);
        Color[] pixels = _texture.GetPixels((int)start.x, (int)start.y, (int)widthX, (int)widthY, 0);
        ...
        _texture.SetPixels((int)start.x, (int)start.y, (int)widthX, (int)widthY, pixels, 0);
    }

    public void PaintLine(Vector2 from, Vector2 to, float rad, Color col, float hardness)
    {
        var width = rad * 2;

        var extent = rad;
        var stY = Mathf.Clamp(Mathf.Min(from.y, to.y) - extent, 0, _texture.height);
        var stX = Mathf.Clamp(Mathf.Min(from.x, to.x) - extent, 0, _texture.width);
        var endY = Mathf.Clamp(Mathf.Max(from.y, to.y) + extent, 0, _texture.height);
        var endX = Mathf.Clamp(Mathf.Max(from.x, to.x) + extent, 0, _texture.width);
        ...
        var sqrRad = rad * rad;
        var sqrRad2 = (rad + 1) * (rad + 1);
        Color[] pixels = _texture.GetPixels((int)stX, (int)stY, (int)lengthX, (int)lengthY, 0);
        var start = new Vector2(stX, stY);
        ...
        _texture.SetPixels((int)start.x, (int)start.y, (int)lengthX, (int)lengthY, pixels, 0);
    }


    public void DrawBezier(BezierPoint[] points, float rad, Color col)
    {
        ...
        var start = new Vector2(Mathf.Clamp(topleft.x, 0, _texture.width), Mathf.Clamp(topleft.y, 0, _texture.height));
        var width = new Vector2(Mathf.Clamp(bottomright.x - topleft.x, 0, _texture.width - start.x), Mathf.Clamp(bottomright.y - topleft.y, 0, _texture.height - start.y));

        Color[] pixels = _texture.GetPixels((int)start.x, (int)start.y, (int)width.x, (int)width.y, 0);
        ...
        _texture.SetPixels((int)start.x, (int)start.y, (int)width.x, (int)width.y, pixels, 0);
        _texture.Apply();
    }
    ...
    public void Clear(Color clearColor)
    {
        for (int y = 0; y < this._texture.height; ++y)
        {
            ...
            {
                this._texture.SetPixel(x, y, clearColor);
            }
        }
    }
}
